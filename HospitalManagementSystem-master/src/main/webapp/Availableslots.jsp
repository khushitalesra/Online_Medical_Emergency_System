<%@ page language="java" contentType="text/html; charset=ISO-8859-1"

    pageEncoding="ISO-8859-1"%>

     <%@ page import="java.sql.*" %>

    <%@ page import="java.io.*" %>

<!DOCTYPE html>

<html>

<head>

<meta charset="ISO-8859-1">

<title>Available Slots</title>
<style>
body{

background-image: url("C:/Users/ABHINAV/Desktop/p1/images/doc14.jpg");


 
height: 100%;

  background-position: center;

  background-repeat: no-repeat;

  background-size: cover;

}
</style>
</head>

<body>

<h2>Your slot at has been booked!</h2>

<br>

<h2>Be there 10min before your slot commence</h2>

<%

String datee= (String) session.getAttribute("date");

String patient= (String) session.getAttribute("name");  //Patients username(Email) that was used to login

//out.println(patient);

String doctor=(String) session.getAttribute("naame");// Doctors name.. used in search

//out.println(doctor);

String time= (String)session.getAttribute("slottype");

//out.println(time);

///out.println(time);

Connection con= null;

 

PreparedStatement ps = null;

 

ResultSet rs = null;

 

 

 

String driverName = "com.mysql.jdbc.Driver";

 

String url = "jdbc:mysql://localhost:3306/patientsprofile";

 

String user = "root";

 

String password = "padamin1";

Connection connection = null;

PreparedStatement ps1=null,ps2=null;

ResultSet rs1 = null, rs2=null;

int updateQuery = 0;

connection = DriverManager.getConnection(url, user, password);

String sql1 ="SELECT * FROM doctorsignup WHERE Name=?";

//out.println(sql1);

ps1=connection.prepareStatement(sql1);

ps1.setString(1,doctor);

rs1 = ps1.executeQuery();

while(rs1.next())

{

                String mail=rs1.getString("Email_Id");

                out.println(mail);

                String queryString = "INSERT INTO appointments(DoctorName,DoctorEmail,PatientEmail,Date,Time)VALUES(?,?,?,?,?)";

                ps = connection.prepareStatement(queryString);

     ps.setString(1,doctor);

    ps.setString(2,mail);

     ps.setString(3,patient);

     ps.setString(4,datee);

     ps.setString(5,time);

     updateQuery = ps.executeUpdate();

     if (updateQuery != 0) { %>

    <br>

    <TABLE style="background-color: #E3E4FA;"

WIDTH="30%" border="1">

               <tr><th>Data is inserted successfully

in database.</th></tr>

            </table>

<%

}

}

 

 

 

 

                // close all the connections.

ps.close();

                ps1.close();

                connection.close();

 

%>

 

</body>

</html>